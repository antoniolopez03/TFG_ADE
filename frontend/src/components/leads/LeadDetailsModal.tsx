...
export function LeadDetailsModal({ lead, onClose }: LeadDetailsModalProps) {
  ...
  const contactoNombre = lead.contacto_nombre_completo?.trim() ?? "";
  const ubicacionEmpresa = [lead.empresa_ciudad, lead.empresa_pais]
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold text-slate-900 dark:text-white">
                {formatFallback(lead.empresa_nombre)}
              </h2>
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                {contactoNombre || "Contacto sin nombre"}
                {lead.contacto_cargo ? ` - ${lead.contacto_cargo}` : ""}
              </p>
            </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50/50 p-5 dark:border-slate-700 dark:bg-slate-800/40">
            ...
            <dl className="space-y-3 text-sm">
              ...
              <div className="grid gap-1">
                ...
                <dd className="font-medium text-slate-900 dark:text-white">{formatFallback(lead.contacto_cargo)}</dd>
              </div>

              <div className="grid gap-1">
                <dt className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Departamento</dt>
                <dd className="mt-1 text-slate-700 dark:text-slate-300">{formatFallback(lead.contacto_departamento)}</dd>
              </div>
                ...
                <div>
                  ...
                  <dd className="mt-1">
                    {lead.contacto_telefono ? (
                      <a
                        href={`tel:${lead.contacto_telefono}`}
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50/50 p-5 dark:border-slate-700 dark:bg-slate-800/40">
            ...
            <dl className="space-y-3 text-sm">
              <div className="grid gap-1">
                ...
                <dd className="font-medium text-slate-900 dark:text-white">{formatFallback(lead.empresa_nombre)}</dd>
              </div>
              ...
              <div className="grid gap-1">
                ...
                <dd className="font-medium text-slate-900 dark:text-white">{formatFallback(lead.empresa_sector)}</dd>
              </div>
              ...
              <div className="grid gap-1 sm:grid-cols-2 sm:gap-3">
                <div>
                  ...
                  <dd className="mt-1 inline-flex rounded-md bg-slate-200/70 px-2 py-1 text-xs font-medium text-slate-700 dark:bg-slate-700/50 dark:text-slate-200">
                    {formatFallback(lead.empresa_empleados_rango)}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Facturacion</dt>
                  <dd className="mt-1 inline-flex rounded-md bg-emerald-100/70 px-2 py-1 text-xs font-medium text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
                    {formatFallback(lead.empresa_facturacion_rango)}
                  </dd>
                </div>
              </div>
              ...
              <div className="grid gap-1 sm:grid-cols-2 sm:gap-3">
                <div>
                  ...
                  <dd className="mt-1 text-slate-700 dark:text-slate-300">{formatFallback(lead.empresa_telefono)}</dd>
                </div>
                <div>
              ...
              <div className="grid gap-1">
                <dt className="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">Descripción</dt>
                <dd className="text-slate-700 dark:text-slate-300">{formatFallback(lead.empresa_descripcion)}</dd>
              </div>
            </dl>
          </section>
        </div>
